  ...
  <section class="v-report-page">
    ...
    <div class="v-report-page__tabs">
      <button
        v-for="tab in tabs"
        :key="tab.key"
        class="v-report-page__tab"
        :class="{ 'v-report-page__tab--active': activeTab === tab.key }"
    ...
    <div class="v-report-page__content">
      ...
      <div v-show="activeTab === 'rapport'" class="v-report-page__main">
        <div class="v-report-page__layout">
          <!-- Sidebar -->
          <app-report-sidebar
            v-if="bodyContentArray.length > 0"
            :body-content="bodyContentArray"
            ...
            <template v-if="bodyContentArray.length > 0">
              <template v-for="(item, index) in bodyContentArray" :key="index">
                ...
                <template v-if="item.content?.type === 'mdheading'">
                  <app-report-heading
                    :level="item.content.content.level"
                ...
                <template v-else-if="item.content?.type === 'mdreportimage'">
                  <app-report-image
                    :image="item.image?.[0]"
                    :size="item.content.content.size || 'large'"
                    :alt="item.content.content.alt"
                    :caption="item.content.content.caption"
                    class="v-report-page__section"
                    :class="{
                ...
                <template v-else-if="item.content?.type === 'list' || item.content?.type === 'listPoints'">
                  <app-report-list
                    :title="item.content.content.titleContent || item.content.content.titlecontent"
                    :text="item.content.content.text"
                    class="v-report-page__section"
                  />
                </template>
      ...
      <div v-show="activeTab === 'bibliographie'" class="v-report-page__bibliography">
        ...
        <div v-if="bibliography && bibliography.length > 0" class="v-report-page__bibliography-list">
          <div
            v-for="(item, index) in bibliography"
            :key="item.id || index"
      ...
      <div v-show="activeTab === 'enlien'" class="v-report-page__related">
        ...
        <div v-if="parsedTags.length > 0" class="v-report-page__tags">
          <nuxt-link
            ...
            :key="tag"
            :href="`/bibliotheque/?q=${encodeURIComponent(tag)}`"
            class="v-report-page__tag"
          >
            {{ tag }}
          </nuxt-link>
        </div>
        ...
        <div v-if="relatedReports.length > 0" class="v-report-page__related-list">
          <app-report-item
            ...
            :key="report.slug"
            :img-src="report.headerImage?.resize?.reg || report.headerImage?.mediaUrl"
            :object-position="report.headerImage?.focus || 'center'"
            :date-label="formatYear(report.dateStart)"
            :title="report.title"
...
const tags: Ref<UnwrapRef<string | undefined>> = ref(undefined)
...
const parsedTags = computed(() => {
  ...
  // Tags are stored as UUIDs, we need to display them properly
  // For now, we'll display them as-is but ideally fetch tag names
  return tags.value.split(',').map((t: string) => t.trim()).filter((t: string) => t.length > 0)
})
